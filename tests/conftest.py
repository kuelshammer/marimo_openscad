...
def mock_wasm_renderer():
    """Mock WASM renderer for testing (legacy - prefer real_wasm_renderer)"""
...
@pytest.fixture
def real_wasm_renderer():
    """Real WASM renderer using actual bundled files (no mocking)"""
    from marimo_openscad.openscad_wasm_renderer import OpenSCADWASMRenderer
    return OpenSCADWASMRenderer()


@pytest.fixture  
def minimal_mock_wasm_renderer():
    """Minimally mocked WASM renderer - only mocks browser execution"""
    from marimo_openscad.openscad_wasm_renderer import OpenSCADWASMRenderer
    from unittest.mock import patch
    
    # Use real renderer but mock only the JavaScript execution part
    renderer = OpenSCADWASMRenderer()
    
    # Mock only the placeholder creation (browser execution would be real)
    original_method = renderer._create_wasm_placeholder
    
    def mock_placeholder(scad_code):
        # Return mock STL data that looks more realistic
        import struct
        
        # Create a minimal binary STL header
        header = b'Generated by OpenSCAD WASM (Test)' + b'\x00' * (80 - 34)
        triangle_count = struct.pack('<I', 1)  # 1 triangle
        
        # One triangle (minimal valid STL)
        normal = struct.pack('<fff', 0.0, 0.0, 1.0)  # Normal vector
        v1 = struct.pack('<fff', 0.0, 0.0, 0.0)      # Vertex 1  
        v2 = struct.pack('<fff', 1.0, 0.0, 0.0)      # Vertex 2
        v3 = struct.pack('<fff', 0.0, 1.0, 0.0)      # Vertex 3
        attrs = struct.pack('<H', 0)                   # Attribute bytes
        
        return header + triangle_count + normal + v1 + v2 + v3 + attrs
    
    with patch.object(renderer, '_create_wasm_placeholder', side_effect=mock_placeholder):
        yield renderer
